...
def generate_careplan_mock(patient, provider, primary_diagnosis, additional_diagnosis,
                          medication_name, medication_history, patient_records):
    """Mock LLM：直接返回固定文本，不调用 OpenAI"""
...
def generate_careplan(patient, provider, primary_diagnosis, additional_diagnosis,
                     medication_name, medication_history, patient_records):
    """
    统一入口：根据 USE_MOCK_LLM 切换 mock / 真实 LLM
    """
    if settings.USE_MOCK_LLM:
        return generate_careplan_mock(
            patient, provider, primary_diagnosis, additional_diagnosis,
            medication_name, medication_history, patient_records
        )
    return generate_careplan_with_llm(
        patient, provider, primary_diagnosis, additional_diagnosis,
        medication_name, medication_history, patient_records
    )
